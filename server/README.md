...
- 리포트처럼 데이터를 비교하거나 계산해서 보여주는 기능은 현재 진행하지 않는다.
- 성장 리포트, 장비 비교, 상세 스냅샷 분석 기능은 알림 기능을 먼저 완성한 뒤 필요하면 후순위로 검토한다.
...
다만 새 기획에서는 알림 기능이 최우선이므로, 이후 구현은 스케줄러/공지 기반 알림 흐름에 맞춰 재정리할 예정이다.
리포트와 데이터 계산 기능은 당장 구현하지 않고, 알림 기능 완성 이후 추가 기능으로 검토한다.
